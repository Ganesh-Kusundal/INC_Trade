...
from brokers.common.enum_utils import canonicalize_order_enums
from brokers.common.idempotency.simple_cache import SimpleIdempotencyCache as IdempotencyCache
from brokers.common.order_validation import high_notional_warning
...
from brokers.dhan.segments import DERIVATIVE_SEGMENTS, DEFAULT_SEGMENT, EQUITY_ONLY_PRODUCTS, EXCHANGE_TO_SEGMENT, segment_to_exchange
...
class OrdersAdapter:
    ...
    def validate_order(
        ...
        if segment in DERIVATIVE_SEGMENTS and inst.lot_size > 1 and quantity % inst.lot_size != 0:
            errors.append(
        ...
        if segment in DERIVATIVE_SEGMENTS and pt_val in EQUITY_ONLY_PRODUCTS:
            errors.append(
    ...
    def validate_order_warnings(
        ...
        return high_notional_warning(quantity, price)
    ...
    def place_order(self, request: BrokerOrderPayload) -> OrderResponse:
        ...
        Returns:
            ``OrderResponse`` — ``ok()`` on success, ``fail()`` on validation
            or risk-check denial (REF-03 unified exception policy).
        """
        ...
        with self._idempotency.lock(correlation_id):
            ...
            if errors:
                ...
                return R.fail(f"Order validation failed: {msg}")
            ...
            side_val, ot_val, pt_val, v_val = canonicalize_order_enums(
                side,
                order_type,
                product_type,
                validity,
            ...
            if self._risk_manager is not None:
                ...
                if not risk_result.allowed:
                    return R.fail(f"Risk check failed: {risk_result.reason}")
