...
class BrokerFactory(BrokerProviderFactory):
    ...
    def _create_connection_and_gateway(
        ...
        # Register Dhan status mappings explicitly (avoids import-time side effect).
        from brokers.dhan.status_mapper import register_mappings as _register_dhan_status
        _register_dhan_status()
    ...
    def _setup_token_refresh_scheduler(
        ...
        def _on_token_refresh(new_token: str) -> None:
            ...
            logger.info(
                ...
                extra={
                    "token_refreshed": bool(new_token),
                    "receivers": delivered,
                },
            )
