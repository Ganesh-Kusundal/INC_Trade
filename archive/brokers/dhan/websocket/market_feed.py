...
class DhanMarketFeed(ReconnectingServiceMixin, ManagedService):
    ...
    def __init__(
        ...
        self._max_reconnect_attempts = int(os.getenv("DHAN_MAX_RECONNECT_ATTEMPTS", "50"))
        self._staleness_threshold = float(os.getenv("DHAN_STALENESS_THRESHOLD_SECONDS", "60.0"))
        self._reconnect_cooldown = float(os.getenv("DHAN_RECONNECT_COOLDOWN_SECONDS", "300"))
        ...
        while not self._stop_event.is_set():
            ...
            if stale_feed is not None:
                ...
                if self._stop_event.wait(timeout=backoff):
                    break
                backoff = min(backoff * 2, max_backoff) + random.uniform(0, 1.0)
                continue
