...
from brokers.common.segment_mapper import SegmentMapper
...
# Single source of truth for which segments are derivatives and which
# product types are equity-only.  Used by OrdersAdapter validation and
# DhanIdentityProvider index-fallback logic.
...
EQUITY_ONLY_PRODUCTS: frozenset[str] = frozenset({"CNC", "MTF"})


class DhanSegmentMapper(SegmentMapper):
    """Dhan implementation of ``SegmentMapper``.

    Wraps the module-level conversion functions so that common code
    can depend on the ABC without importing module-level names directly.
    """
    ...
    def from_wire(self, wire: str) -> ExchangeSegment:
        return from_sdk_int(wire) if wire.isdigit() else parse_segment(wire)  # type: ignore[return-value]

    def exchange_to_wire(self, exchange: str, default: str = DEFAULT_SEGMENT) -> str:
        return EXCHANGE_TO_SEGMENT.get(exchange, default)
