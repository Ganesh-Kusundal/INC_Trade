...
class StreamOrchestrator:
    ...
    def _normalize_tick(
        ...
        return MarketTick(
            ...
            ltp=Decimal(str(frame.get("ltp") or frame.get("last_price") or 0)),
            volume=int(frame.get("volume") or 0),
            bid=Decimal(str(frame["bid"])) if "bid" in frame else None,
            ask=Decimal(str(frame["ask"])) if "ask" in frame else None,
            broker_id=broker_id,
            session_id=session_id,
            event_time=now,
            sequence=frame.get("sequence"),
            open=Decimal(str(frame["open"])) if "open" in frame else None,
            high=Decimal(str(frame["high"])) if "high" in frame else None,
            low=Decimal(str(frame["low"])) if "low" in frame else None,
        )
    ...
    def _normalize_order_update(
        ...
        return OrderUpdate(
            ...
            avg_price=Decimal(str(frame.get("avg_price") or 0)),
            raw=frame,
        )
