...
class PaperGateway(BatchFetchMixin, MarketDataGateway, ObservabilityProvider):
    ...
    def option_chain(
        ...
        base = self._market_data.get_ltp(underlying, "NSE")
        base_float = float(base)
        strikes = [round(base_float + i * 50, 0) for i in range(-10, 11)]
        chain = []
        for strike in strikes:
            chain.append(
                {
                    "strike": strike,
                    "call": {"ltp": round(max(0, base_float - strike + np.random.uniform(5, 50)), 2)},
                    "put": {"ltp": round(max(0, strike - base_float + np.random.uniform(5, 50)), 2)},
                }
            )
    ...
    def future_chain(
        ...
        base = float(self._market_data.get_ltp(underlying, "NSE"))  # Decimal→float for numpy sim
