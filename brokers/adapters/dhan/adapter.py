...
DhanGateway, breaking the circular dependency between ``brokers/`` and
``inc_trade/``.
...
class DhanAdapter:
    ...
    Composes Dhan sub-adapters without using the DhanGateway, breaking
    the circular dependency between ``brokers/`` and ``inc_trade/``.
