"""``python -m brokers.adapters.upstox.auth.login`` — interactive Upstox OAuth flow.
...
Usage::
    ...
    UPSTOX_CLIENT_ID=xxx UPSTOX_CLIENT_SECRET=yyy \
        python -m brokers.adapters.upstox.auth.login
"""
...
def build_auth_url(
    ...
    base = Upstox.SANDBOX_V2 if settings.is_sandbox else Upstox.PROD_V2
...
def perform_login(
    ...
    *,
    ...
    """
    code = asyncio.run(
        _capture_code(settings, timeout=timeout, open_browser=open_browser)
    )
...
def _persist_state(settings: UpstoxConnectionSettings, result: dict[str, Any]) -> None:
    ...
    expires_at_ms = (
        int(time.time() * 1000) + int(result.get("expires_in_seconds", 86400)) * 1000
    )
    state = {
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger.info(
        "authorization_complete", extra={"token_file": str(settings.token_state_file)}
    )
    print()
