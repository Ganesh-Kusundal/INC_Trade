...
Rate limits: Per-endpoint (orders=25/s, market data=10/s)
...
Usage::
    ...
    data = client.get("/v2/order/retrieve-all")
"""
...
class UpstoxHttpClient(BaseHttpClient):
    ...
    def place_order(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        return self.post("/v2/order/place", json=payload)

    def modify_order(self, order_id: str, changes: dict[str, Any]) -> dict[str, Any]:
        """PUT /v2/order/modify — modify an existing order."""
        payload = {"order_id": order_id, **changes}
        return self.put("/v2/order/modify", json=payload)

    def cancel_order(self, order_id: str) -> dict[str, Any]:
        """DELETE /v2/order/cancel — cancel an order."""
        return self.delete(f"/v2/order/cancel?order_id={order_id}")

    def get_order(self, order_id: str) -> dict[str, Any]:
        """GET /v2/order/history — get order history (includes status)."""
        return self.get(f"/v2/order/history?order_id={order_id}")

    def get_orderbook(self) -> dict[str, Any]:
        """GET /v2/order/retrieve-all — fetch all orders."""
        return self.get("/v2/order/retrieve-all")

    def get_trades(self) -> dict[str, Any]:
        """GET /v2/trades/trades-for-day — fetch today's trades."""
        return self.get("/v2/trades/trades-for-day")

    # ── Market data endpoints ───────────────────────────────────────────────

    def get_ltp(self, instrument_keys: list[str]) -> dict[str, Any]:
        """GET /v2/market-quote/ltp — get last traded price."""
        keys = ",".join(instrument_keys)
        return self.get(f"/v2/market-quote/ltp?instrument_key={keys}")

    def get_quote(self, instrument_keys: list[str]) -> dict[str, Any]:
        """GET /v2/market-quote/quotes — get full quote."""
        keys = ",".join(instrument_keys)
        return self.get(f"/v2/market-quote/quotes?instrument_key={keys}")

    def get_ohlc(self, instrument_keys: list[str]) -> dict[str, Any]:
        """GET /v2/market-quote/ohlc — get OHLC data."""
        keys = ",".join(instrument_keys)
        return self.get(f"/v2/market-quote/ohlc?instrument_key={keys}")
