...
from brokers.adapters.upstox.auth.pkce import UpstoxPkceUtil
...
def test_pkce_challenge_is_url_safe_base64_no_padding():
    pair = UpstoxPkceUtil.generate()
    # Decode helper padding matching standard base64 rules
    pad = "=" * (4 - (len(pair.code_challenge) % 4)) if len(pair.code_challenge) % 4 else ""
    decoded = base64.urlsafe_b64decode(pair.code_challenge + pad)
