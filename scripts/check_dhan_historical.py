...
def main():
    ...
    print("Initializing Dhan Broker Facade...")
    ...
    try:
        # We pass dates as ISO strings as expected by the facade
        ...
        if not candles:
            print("No candles returned. (Is the market closed for the entire duration, or invalid credentials?)")
        else:
            print(f"\nSuccess! Retrieved {len(candles)} candles.")
            print("Showing the last 5 candles:")
            for candle in candles[-5:]:
                print(f"  Time: {candle.timestamp}, Open: {candle.open}, High: {candle.high}, Low: {candle.low}, Close: {candle.close}, Vol: {candle.volume}")
