...
This service is a :class:`brokers.infrastructure.lifecycle.ManagedService`.
"""
...
from brokers_core.domain.lifecycle_health import HealthStatus
from brokers_core.infrastructure.lifecycle import HealthState, ManagedService, build_health
        ...
        try:
            if self._token_manager.settings.is_totp:
                ...
                try:
                    buffer_minutes = int(
                        getattr(
                            self._token_manager.settings, "refresh_buffer_minutes", 30
                        )
                        or 30
                    )
