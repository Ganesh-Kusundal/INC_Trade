...
from brokers_core.domain.constants.timeouts import DEFAULT_HTTP_TIMEOUT_SECONDS
...
class UpstoxConnectionSettings:
    ...
    instrument_cache: Path = field(
        default_factory=lambda: Path(".cache/upstox/complete.json.gz")
    )
    ...
    def base_v2(self) -> str:
        ...
        if self.is_sandbox:
            return Upstox.SANDBOX_V2
        return Upstox.PROD_V2

    @property
    def base_hft(self) -> str:
        if self.is_sandbox:
            return Upstox.SANDBOX_HFT
        return Upstox.PROD_HFT
...
class UpstoxSettingsLoader(SettingsLoaderBase):
    ...
    def from_env(
        ...
        *,
        ...
        client_secret = (
            cls._get(prefix, "SANDBOX_CLIENT_SECRET")
            or cls._get(prefix, "CLIENT_SECRET")
        ) or ""
        access_token = (
        ...
        market_protection_default = cls._get_int(
            prefix, "MARKET_PROTECTION_DEFAULT", default=-1
        )
        redirect_port = cls._get_int(prefix, "REDIRECT_PORT", default=18080)
        refresh_buffer_minutes = cls._get_int(
            prefix, "REFRESH_BUFFER_MINUTES", default=30
        )
    ...
    @classmethod
    def from_properties(
        cls, path: Path, *, prefix: str = "upstox"
    ) -> UpstoxConnectionSettings:
        ...
        return UpstoxConnectionSettings(
            ...
            analytics_only=cls._parse_bool(
                values.get(f"{prefix}.analyticsOnly"), False
            ),
            auth_mode=auth_mode,
            environment=environment,
            ...
            allow_live_orders=cls._parse_bool(
                values.get(f"{prefix}.allowLiveOrders"), False
            ),
            ...
            totp_refresh_hour=cls._parse_int(
                values.get(f"{prefix}.totpRefreshHour"), 8
            ),
            totp_refresh_minute=cls._parse_int(
                values.get(f"{prefix}.totpRefreshMinute"), 0
            ),
        )
