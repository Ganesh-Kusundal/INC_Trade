...
from brokers_core.domain import Order, OrderResponse
...
from brokers_core.adapters.upstox.config import PRODUCT_TYPE_MAP, VALIDITY_MAP
from brokers_core.adapters.upstox.instruments import UpstoxInstruments, resolve_upstox_instrument_key
from brokers_core.adapters.upstox.mapper import map_order, map_order_response, unwrap_data
...
class UpstoxOrders:
    def __init__(
        ...
        self._idempotency_cache = idempotency_cache or TypedIdempotencyCache()
    ...
    def place_order(
        ...
        payload = {
            ...
            "order_type": _WIRE_MAPPER.map_order_type(order_type),
            "transaction_type": _WIRE_MAPPER.map_side(side),
            "disclosed_quantity": 0,
            "trigger_price": wire_price_or_zero(trigger_price),
            "is_amo": is_amo,
        }
        if correlation_id:
    ...
    def modify_order(
        ...
        if price is not None:
            payload["price"] = wire_price_or_zero(price)
        if order_type is not None:
            payload["order_type"] = _WIRE_MAPPER.map_order_type(order_type)
