...
from brokers_core.adapters.paper.capabilities import paper_capabilities
from brokers_core.domain import (
    Balance,
    Holding,
    MarketDepth,
    Order,
...
from brokers_core.domain.enums import BrokerID, OrderStatus, OrderType, ProductType, Validity
from brokers_core.ports import (
    InstrumentInfo,
)
from brokers_core.ports.extension_registry import ExtensionRegistryPort
...
class _PaperMarketData:
    ...
    def ltp_batch(self, symbols: list[str], exchange: str = DEFAULT_EQUITY_EXCHANGE) -> dict[str, Decimal]:
...
class _PaperInstruments:
    ...
    def resolve(self, symbol: str, exchange: str = DEFAULT_EQUITY_EXCHANGE) -> InstrumentInfo | None:
...
class _PaperOptions:
    def get_expiries(self, underlying: str, exchange: str = DEFAULT_DERIVATIVE_EXCHANGE) -> list[str]:
        from brokers_core.domain.exceptions import NotSupportedError
...
class PaperGateway:
    ...
    def extensions(self) -> ExtensionRegistryPort:
        ...
        if not hasattr(self, "_extension_registry"):
            from brokers_core.ports.extension_registry import DictExtensionRegistry
