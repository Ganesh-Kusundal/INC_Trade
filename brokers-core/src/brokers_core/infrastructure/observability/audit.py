...
4. An :class:`AuditLogger` that auto-injects correlation IDs from context.
...
Usage in broker infrastructure code::
    ...
    from brokers_core.infrastructure.observability.audit import AuditLogger, emit_routing_decision

    audit = AuditLogger()
    audit.emit_routing_decision(decision)
    audit.emit_quota_event(token, wait_ms=12.3)
    audit.emit_stream_state_change(session, from_state, to_state, reason)

    # Or use module-level emitters (backward compatible):
    emit_routing_decision(decision)
    emit_quota_event(broker_id, endpoint_class, priority_class, "acquire")
"""
...
from brokers_core.infrastructure.correlation import get_current_correlation_id
...
@dataclass
class AuditEvent:
    """Base audit event with automatic correlation ID injection.

    All audit events inherit from this base. The ``correlation_id`` is
    automatically populated from the current context when the event is
    created, and the ``timestamp`` is set to the current UTC time.
    """

    event: str
    correlation_id: str = field(default="", init=False)
...
@dataclass
class RoutingDecisionEvent(AuditEvent):
    ...
    parallel_brokers: list[str] = field(default_factory=list)
    policy_version: str = ""
...
@dataclass
class HistoricalChunkEvent(AuditEvent):
...
@dataclass
class HistoricalMergeConflictEvent(AuditEvent):
...
@dataclass
class StreamStateChangeEvent(AuditEvent):
...
@dataclass
class StreamFailoverEvent(AuditEvent):
...
@dataclass
class ExtensionResolveEvent(AuditEvent):
    """Emitted on every extension registry lookup."""

    broker_id: str = ""
    extension_name: str = ""
    hit: bool = False  # True if extension was found
...
@dataclass
class DegradedModeEvent(AuditEvent):
...
class AuditLogger:
    """Structured audit logger with automatic correlation ID injection.

    Wraps all event emitters and ensures every event carries the current
    correlation ID from context. This is the primary interface for emitting
    audit events in new code.

    Usage::

        audit = AuditLogger()
        audit.emit_routing_decision(decision)
        audit.emit_quota_event("dhan", "execution", "CRITICAL", "acquire")
    """

    def __init__(self, logger_name: str = "broker.audit") -> None:
        self._logger = logging.getLogger(logger_name)

    def _emit(self, event_obj: Any) -> None:
        """Emit a structured audit event.

        Converts the event dataclass to a dictionary and logs it with
        the structured logger. The correlation_id and timestamp are
        automatically included via the AuditEvent base class.
        """
        ...
        if event_name in ("stream.failover", "degraded_mode"):
            severity = d.get("severity", "warning")
            level = logging.CRITICAL if severity == "critical" else logging.WARNING
        elif event_name == "extension.resolve":
            level = logging.DEBUG if d.get("hit") else logging.INFO
        else:
            level = logging.INFO

        self._logger.log(level, event_name, extra=d)

    def emit_routing_decision(self, decision: Any) -> None:
        ...
        evt = RoutingDecisionEvent(
            ...
            primary_broker=getattr(decision, "primary_broker", ""),
            fallback_brokers=list(getattr(decision, "fallback_brokers", [])),
            parallel_brokers=list(getattr(decision, "parallel_brokers", [])),
            policy_version=getattr(decision, "policy_version", ""),
            reason_codes=list(getattr(decision, "reason_codes", [])),
            rejected=dict(getattr(decision, "rejected", {})),
            decided_at=decision.decided_at.isoformat()
            if hasattr(decision.decided_at, "isoformat")
            else str(getattr(decision, "decided_at", "")),
        )
        self._emit(evt)
    ...
    def emit_historical_chunk(
        ...
        """Emit a historical chunk fetch event."""
        self._emit(
    ...
    def emit_merge_conflict(
        ...
        """Emit a historical merge conflict event."""
        self._emit(
    ...
    def emit_stream_state_change(
        ...
        """Emit a stream session state change event."""
        self._emit(
    ...
    def emit_extension_resolve(
        ...
        """Emit an extension resolve event."""
        self._emit(
...
# ---------------------------------------------------------------------------
# Module-level convenience emitters (backward compatible)
# ---------------------------------------------------------------------------
...
def emit_routing_decision(decision: Any) -> None:
    """Emit a structured routing decision event."""
    _default_audit.emit_routing_decision(decision)
...
def emit_quota_event(
    ...
    """Emit a quota acquire or reject event."""
    _default_audit.emit_quota_event(
        broker_id, endpoint_class, priority_class, event_type,
        wait_ms, token_id, retry_after_s,
    )
...
def emit_historical_chunk(
    ...
    """Emit a historical chunk fetch event."""
    _default_audit.emit_historical_chunk(
        request_id, chunk_id, broker_id, from_date, to_date,
        timeframe, event_type, bar_count, latency_ms, error,
    )
...
def emit_merge_conflict(
    ...
    """Emit a historical merge conflict event."""
    _default_audit.emit_merge_conflict(
        request_id, instrument, timeframe, bar_event_time,
        primary_broker, secondary_broker, delta_pct, resolution,
    )
...
def emit_stream_state_change(
    ...
    """Emit a stream session state change event."""
    _default_audit.emit_stream_state_change(
        session_id, broker_id, stream_kind, from_state,
        to_state, reason, reconnect_generation,
    )
...
def emit_stream_failover(
    ...
    """Emit a stream failover event."""
    _default_audit.emit_stream_failover(
        session_id, primary_broker, fallback_broker,
        stale_seconds, handoff_ms,
    )
...
def emit_extension_resolve(
    ...
    """Emit an extension resolve event."""
    _default_audit.emit_extension_resolve(
        broker_id, extension_name, hit, alternatives,
    )
...
def emit_degraded_mode(
    ...
    """Emit a degraded mode event."""
    _default_audit.emit_degraded_mode(subsystem, reason, broker_id, severity)
...
broker_errors_total{broker, operation, error_class}
    - Counter: total errors per broker per operation per error type
      (error_class: timeout, auth, quota, network, broker-error).
