...
class TestPortStructure:
    ALL_PORTS = [
        "brokers_core.ports.auth.AuthPort",
        "brokers_core.ports.clock.ClockPort",
        "brokers_core.ports.connection_lifecycle.ConnectionLifecyclePort",
        "brokers_core.ports.historical.HistoricalPort",
        "brokers_core.ports.instruments.InstrumentPort",
        "brokers_core.ports.market_data.MarketDataPort",
        "brokers_core.ports.order_execution.OrderExecutionPort",
        "brokers_core.ports.portfolio.PortfolioPort",
        "brokers_core.ports.risk_manager.RiskManagerPort",
        "brokers_core.ports.streaming.StreamHandle",
        "brokers_core.ports.streaming.StreamingPort",
        "brokers_core.ports.token_store.TokenStorePort",
        "brokers_core.ports.capabilities.MarginProvider",
        "brokers_core.ports.capabilities.SuperOrderProvider",
        "brokers_core.ports.capabilities.ForeverOrderProvider",
        "brokers_core.ports.capabilities.KillSwitchProvider",
        "brokers_core.ports.capabilities.SliceOrderProvider",
        "brokers_core.ports.capabilities.NewsProvider",
        "brokers_core.ports.options.OptionsPort",
        "brokers_core.ports.extension_registry.ExtensionRegistryPort",
    ]
    ...
    def test_all_core_ports_exported(self) -> None:
        import brokers_core.ports

        for dotted_path in self.ALL_PORTS:
            cls_name = dotted_path.rsplit(".", 1)[-1]
            assert hasattr(brokers_core.ports, cls_name), (
                f"{cls_name} not exported from brokers_core.ports"
            )
...
class TestExceptionHierarchy:
    ...
    def test_all_exceptions_inherit_tradexv2_error(self) -> None:
        from brokers_core.domain import exceptions
        from brokers_core.domain.exceptions import TradeXV2Error
...
class TestErrorCodeCoverage:
    def test_all_constants_referenced(self) -> None:
        ...
        from brokers_core.domain import exceptions as it_exceptions
...
class TestCapabilityConstants:
    ...
    def test_all_capabilities_in_all_caps(self) -> None:
        from brokers_core.domain.constants.capabilities import ALL_CAPABILITIES
...
class TestExtensionPackage:
    ...
    def test_all_extension_protocols_exported(self) -> None:
        # Extensions are not part of brokers-core, skip this test
        pass
