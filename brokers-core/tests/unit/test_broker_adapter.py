...
class TestAdapterArchitecture:
    ...
    def test_adapter_imports_gateway_only_at_runtime(self) -> None:
        """Importing brokers_core.adapters.dhan should NOT import the gateway
        ...
        # Re-import brokers_core.adapters.dhan (remove from cache to force fresh)
        for mod in list(sys.modules.keys()):
            if mod.startswith("brokers_core.adapters.dhan"):
                sys.modules.pop(mod, None)

        importlib.import_module("brokers_core.adapters.dhan")
        ...
        assert not new_gateway_modules, (
            f"brokers_core.adapters.dhan eagerly loaded gateway modules: {new_gateway_modules}"
        )
